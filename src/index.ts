...
const BASE = 'https://opendata.utah.gov';
const UA = 'pipeworx-mcp-data-utah/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Utah Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Utah Open Data dataset by resource_id (e.g. "7n5d-er3z"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "7n5d-er3z" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Utah Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "7n5d-er3z".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "7n5d-er3z".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"7n5d-er3z"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"7n5d-er3z"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`opendata.utah.gov: ${res.status}`);
  return res.json();
}
